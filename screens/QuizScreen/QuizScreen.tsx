...
import {generateQuestionAlbum, generateQuestionTrack} from "../../services/questionManager";
...
export default function QuizScreen() {
  ...
  var initialColors=[styles.button,styles.button,styles.button,styles.button]
  ...
  function onResponse(e,f){
   ...
   setTimeout(()=>{})
    if(alter==false){
          ...
          setTimeout(()=>{setQuestion(generateQuestionTrack(data2));},1000)
          setAlter(true)
          ...
          setTimeout(()=>{setQuestion(generateQuestionAlbum(data));},1000)
          setAlter(false)
...
if(lifes>0){
  return (
    <View>
      ...
      <View style={styles.container}>
        ...
        <ScrollView>
        <Pressable style={color[0]} onPress={()=>{onResponse(question.answers[3],0)}}><Text style={styles.textAwnser}>{question.answers[3]}</Text></Pressable>
        <Pressable style={color[1]} onPress={()=>{onResponse(question.answers[2],1)}}><Text style={styles.textAwnser}>{question.answers[2]}</Text></Pressable>
        <Pressable style={color[2]} onPress={()=>{onResponse(question.answers[1],2)}}><Text style={styles.textAwnser}>{question.answers[1]}</Text></Pressable>
        <Pressable style={color[3]} onPress={()=>{onResponse(question.answers[0],3)}}><Text style={styles.textAwnser}>{question.answers[0]}</Text></Pressable>
        </ScrollView>
    </View>
  ...
  return(
    <View>
        ...
        <Image source={{uri:'https://media1.giphy.com/media/VM01S5yIaKCgqg1bSF/giphy.gif?cid=ecf05e47wsx84h8uxwgbb63qrft1wc6vv3uij6si6micx29r&rid=giphy.gif&ct=g'}}style={styles.image}/>
        ...
        <Pressable style={styles.button}  onPress={() => { setLifes(3);setScore(0);}}>
          <View >
